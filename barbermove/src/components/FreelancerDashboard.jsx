// Dashboard do Freelancer - BarberMove
import React, { useState, useEffect, useCallback } from 'react';
import ScreenWrapper from './ScreenWrapper';
import { 
  Home, MapPin, Upload, Image as ImageIcon, Star,
  DollarSign, Pause, Play, Check, X, Calendar,
  TrendingUp, Award, Users, ClipboardList, UserRound, LogOut
} from 'lucide-react';
import { useApp } from '../contexts/AppContext';
import { Button, Input, Badge, RatingStars, Modal, Loading, AgendamentoCard, BarbeariaCard } from './Common';

export default function FreelancerDashboard() {
  const { apiRequest, notify, user, logout } = useApp();
  ...
  if (view === 'solicitacoes') {
    return (
      <ScreenWrapper>
        <div className="min-h-screen bg-black text-white pb-20">
          <div className="bg-gradient-to-r from-zinc-900 to-zinc-950 p-4 border-b border-zinc-800 flex items-center justify-between">
            <div>
              <h1 className="text-xl font-black">Solicitações</h1>
              <p className="text-zinc-400 text-sm">Pedidos e convites recentes</p>
            </div>
          ...
          <div className="p-4 space-y-3">
            {solicitacoes.length === 0 ? (
              ...
              solicitacoes.map((sol) => <SolicitacaoCard key={sol.id} solicitacao={sol} />)
            )}
          </div>
        </div>
      </ScreenWrapper>
      ...
      <ScreenWrapper>
        <div className="min-h-screen bg-black text-white pb-20">
          <div className="bg-gradient-to-r from-zinc-900 to-zinc-950 p-4 border-b border-zinc-800">
            ...
            <p className="text-zinc-400 text-sm">Resumo da sua conta e acesso</p>
          </div>

          <div className="p-4 space-y-4">
            <div className="bg-[#1e1e24] rounded-2xl p-4 shadow-md border border-zinc-800/50">
              <div className="flex items-center gap-3">
                <div className="h-12 w-12 rounded-full bg-orange-500/15 border border-orange-500/30 flex items-center justify-center text-orange-400 font-black">
                  {String(user?.nome || 'F').charAt(0).toUpperCase()}
                </div>
                <div>
                  <div className="font-bold">{user?.nome || 'Freelancer'}</div>
                  <div className="text-sm text-zinc-400">{user?.email || 'Conta conectada'}</div>
                </div>
              </div>
            </div>

            <div className="bg-[#1e1e24] rounded-2xl p-4 shadow-md border border-zinc-800/50 space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-zinc-400">Status</span><span className="font-semibold">{statusPausado ? 'Pausado' : 'Ativo'}</span></div>
              <div className="flex justify-between"><span className="text-zinc-400">Portfólio</span><span className="font-semibold">{portfolio.length} fotos</span></div>
              <div className="flex justify-between"><span className="text-zinc-400">Barbearias</span><span className="font-semibold">{barbearias.length} próximas</span></div>
            </div>

            <button onClick={logout} className="w-full rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm font-black text-red-300 flex items-center justify-center gap-2">
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="bg-zinc-900 rounded-xl p-4 shadow-md border border-zinc-800">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
                  <span className="text-zinc-400">Via BarberMove:</span>
...
function BottomNav({ active, onChange, type }) {
  const items = {
    freelancer: [
      ...
      { id: 'solicitacoes', label: 'Chamados', icon: ClipboardList },
      { id: 'portfolio', label: 'Fotos', icon: ImageIcon },
      { id: 'ganhos', label: 'Carteira', icon: DollarSign },
