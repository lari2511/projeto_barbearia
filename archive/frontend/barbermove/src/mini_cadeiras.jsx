		...
		root.render(
			<div className="min-h-screen bg-black text-white flex justify-center bm-app-frame">
				<div className="w-full max-w-[430px] min-h-screen p-4 bm-shell-content app-container client-dashboard-shell">
					<Header title={<><span className="mr-2">🪑</span>Gerenciar Cadeiras</>} actionButton={{ label: 'Atualizar', onClick: () => window.location.reload() }} />
					<LiberarCadeirasComponent token={token} API_URL={API_URL} notify={notify} barbeariaId={2} />
				</div>
			</div>
		);
});
